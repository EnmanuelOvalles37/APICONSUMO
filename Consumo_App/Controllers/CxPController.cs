    ...
    namespace Consumo_App.Controllers
    {
        ...
        SELECT 
            ...
            CAST(ISNULL(SUM(MontoBruto), 0) AS DECIMAL(18,2)) AS TotalBruto,
            CAST(ISNULL(SUM(MontoComision), 0) AS DECIMAL(18,2)) AS TotalComision,
            CAST(ISNULL(SUM(MontoPendiente), 0) AS DECIMAL(18,2)) AS TotalPorPagar,
            CAST(ISNULL(SUM(CASE WHEN FechaVencimiento < @Hoy THEN MontoPendiente ELSE 0 END), 0) AS DECIMAL(18,2)) AS TotalVencido
        FROM CxpDocumentos
        ...
        SELECT CAST(ISNULL(SUM(MontoComision), 0) AS DECIMAL(18,2))
        FROM Consumos 
        ...
        SELECT TOP 10
            ...
            CAST(ISNULL(p.PorcentajeComision, 0) AS DECIMAL(18,2)) AS PorcentajeComision,
            CAST(COUNT(*) AS INT) AS Documentos,
            CAST(ISNULL(SUM(d.MontoBruto), 0) AS DECIMAL(18,2)) AS MontoBruto,
            CAST(ISNULL(SUM(d.MontoComision), 0) AS DECIMAL(18,2)) AS MontoComision,
            CAST(ISNULL(SUM(d.MontoPendiente), 0) AS DECIMAL(18,2)) AS MontoPorPagar
        ...
        public async Task<IActionResult> ListarDocumentos(int proveedorId, [FromQuery] bool? soloActivos)
        {
            ...
            // Verificar proveedor con CAST explícitos
            ...
            Rnc, 
            DiasCorte, 
            CAST(ISNULL(PorcentajeComision, 0) AS DECIMAL(18,2)) AS PorcentajeComision 
          FROM Proveedores 
        ...
        SELECT 
            ...
            d.PeriodoDesde,
            d.PeriodoHasta,
            CAST(d.CantidadConsumos AS INT) AS CantidadConsumos,
            CAST(ISNULL(d.MontoBruto, 0) AS DECIMAL(18,2)) AS MontoBruto,
            CAST(ISNULL(d.MontoComision, 0) AS DECIMAL(18,2)) AS MontoComision,
            CAST(ISNULL(d.MontoTotal, 0) AS DECIMAL(18,2)) AS MontoTotal,
            CAST(ISNULL(d.MontoPagado, 0) AS DECIMAL(18,2)) AS MontoPagado,
            CAST(ISNULL(d.MontoPendiente, 0) AS DECIMAL(18,2)) AS MontoPendiente,
            d.Estado,
            d.FechaEmision,
            d.FechaVencimiento,
            CAST(CASE WHEN d.FechaVencimiento < GETUTCDATE() 
                 THEN DATEDIFF(DAY, d.FechaVencimiento, GETUTCDATE()) 
                 ELSE 0 END AS INT) AS DiasVencido
        FROM CxpDocumentos d
        {whereClause}
        ORDER BY d.FechaEmision DESC";
            ...
            var documentos = rawData.Select(d => new
            {
                ...
                NumeroFacturaProveedor = (string?)d.NumeroFacturaProveedor,
                PeriodoDesde = (DateTime)d.PeriodoDesde,
                PeriodoHasta = (DateTime)d.PeriodoHasta,
                CantidadConsumos = (int)d.CantidadConsumos,
                MontoBruto = (decimal)d.MontoBruto,
                MontoComision = (decimal)d.MontoComision,
                ...
                FechaEmision = (DateTime)d.FechaEmision,
                FechaVencimiento = (DateTime)d.FechaVencimiento,
